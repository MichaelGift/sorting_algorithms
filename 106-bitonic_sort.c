...
/**
* swap_int - swaps index's of array
...
/**
* recusive_b_sort - bitonic recursive sort
* @array: array to sort
* @low: lowest index
* @count: Count of slice
* @dir: Direction, ascending 1 descending 0
...
void recusive_b_sort(int *array, int low, int count, int dir, size_t size)
{
	int n;
	...
	if (count > 1)
	{
		...
		recusive_b_sort(array, low, n, 1, size);
		recusive_b_sort(array, low + n, n, 0, size);
...
void bitonic_sort(int *array, size_t size)
{
	if (array == NULL || size < 2)
		return;
	recusive_b_sort(array, 0, size, 1, size);
}
